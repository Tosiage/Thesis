...
Für mit der Polaris durchgeführte Kalibrierung $(p^{pol},q^{pol})$ wurde für jedes $i$ die euklidische Distanz zu $\overline{p_i}$ bestimmt, sowie der Winkel $w_{ai}$ zwischen den mit der Polaris erhaltenen Werten für $e_a^{pol}$ und $\overline{e_{ai}}$. 
...
\begin{tabular}{lllll}
\hline
\textbf{ImageTarget $i$}  & \textbf{Abstand}  & \textbf{$\omega_{xi}$}  & \textbf{$\omega_{yi}$} & \textbf{$\omega_{xi}$}    \\
\hline
&\\
...
\hline
\end{tabular}
\caption[Polaris zu Durchschnitt]{Abstand: euklidische Distanz zwischen $p^{pol}$ und $\overline{p_i}$ in cm;  $\omega_{xi}$: Winkel zwischen $e^{pol}_{xi}$ und $\overline{e_{xi}}$ in Grad; $\omega_{yi}$: Winkel zwischen $e^{pol}_{yi}$ und $\overline{e_{yi}}$ in Grad; $\omega_{zi}$: Winkel zwischen $e^{pol}_{zi}$ und $\overline{e_{zi}}$ in Grad}
\label{fig:5.8}
\end{figure}
